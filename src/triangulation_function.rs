...
impl Location{
    ...
    pub fn distance_to(&self, other: &Location) -> f64{
        ...
        let a = (delta_phi/2.0).sin() * (delta_phi/2.0) + phi1.cos() * phi2.cos() * (delta_lambda/2.0).sin() * (delta_lambda/2.0).sin();
...
pub fn to_lat_lng(v: Vec<f64>) -> Location {
    ...
    let lng = y.atan2(x);
    Location{lat: lat.to_degrees(),lon: lng.to_degrees()}
}
...
mod tests {
    ...
    fn test_vector_folding(){
        let v: Vec<Location> = vec![Location{lat: 1.5, lon: 2.5}, Location{lat: 2.5, lon: 3.5}];
    ...
    fn test_location_interpolation(){
        ...
        let fx1 = Location{lat: 47.62, lon: -122.33};
        let fx2 = Location{lat: 61.2, lon: -149.9};
        assert_eq!(interpolate_location(x, x1, x2, fx1, fx2), Location{lat: 54.41, lon: -136.115});
        let x = 1.9;
        
        let fx1 = Location{lat: 47.62, lon: -122.33};
        let fx2 = Location{lat: 61.2, lon: -149.9};
        assert_eq!(interpolate_location(x, x1, x2, fx1, fx2), Location{lat: 59.842, lon: -147.143});
    }
}
