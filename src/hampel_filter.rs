...
impl MeanFilter for Vec<f64> {
    type Output = Vec<f64>;
    type Inner = f64;
    fn hampel_filter(&self) -> Vec<f64> {
        let mut data = self.clone();
        let gaussian_scale = 1.4826;
        let median = self.median();
        let mad = gaussian_scale
            * data
                .clone()
                .into_iter()
                .map(|x| (x - median).abs())
...
mod tests {
    ...
    fn test_hampel_filter() {
        let mut vec_hampel = vec![
            1.0, 3.0, 5.0, 9.0, 12.0, 5.0, 11.0, 50.0, 20.0, 17.5, 70.0, 12.5,
        ];
        // assert median
        let median = vec_hampel.median();
        // assert two elements (50.0, and 70.0)
        vec_hampel = vec_hampel.hampel_filter();
