...
impl TriangulationFunction for EWAF {
    fn triangulate(bearings: Vec<Bearing>) -> Result<Location, TriangulationError> {
        let location_vec: Vec<Location> = bearings
            .into_iter()
            .combinations(2)
            .into_iter()
            .filter_map(|x| {
                let ret_val;
                let val_first =
                    || -> Option<Location> { triangulate_bearings_pair(&x[0], &x[1]).ok() };
                let val_second =
                    || -> Option<Location> { triangulate_bearings_pair(&x[1], &x[0]).ok() };
...
mod tests {
    ...
    fn test_real_triangulation() {
        assert_eq!(
            Location {
                lat: 50.824255556259786,
                lon: 8.784189999999997
            }
    ...
    fn test_real_triangulation_inf() {
        let result = EWAF::triangulate(vec![
            Bearing {
                loc: Location {
                    lat: 50.814781,
                    lon: 8.769190,
    ...
    fn test_great_circle() {
        assert_eq!(
            vec!("-0.794".to_owned(), "0.129".to_owned(), "0.594".to_owned()),
            great_circle(Bearing {
                loc: Location {
                    lat: 53.3206,
                    lon: -1.7297
                },
    ...
    fn test_to_lat_lang() {
        assert_eq!(
            ...
            .to_string(),
            to_lat_lng(vec!(0.5000, 0.5000, 0.7071)).to_string()
        )
    }
    ...
    fn test_vector_folding() {
        assert_eq!(
            Location { lat: 5.0, lon: 4.0 },
            fold_location_vec(vec![
                Location {
                    lat: 8.52,
                    lon: 7.1
                },
