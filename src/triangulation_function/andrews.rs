...
impl TriangulationFunction for Andrew {
    fn triangulate(b: Vec<Bearing>) -> Result<Location, TriangulationError> {
            ...
            && counter <= 999
        {
            ...
            let psi_a: Vec<f64> = t
                .iter()
                .map(|te| match te.abs() < 1.5 * std::f64::consts::PI {
                    true => 1.5 * (te / 1.5).sin(),
                    false => 0.0,
                })
                .collect();
...
mod tests {
    ...
    fn test_triangulation() {
        ...
        assert_eq!(
            ...
            triangulate::<Andrew>(base_sample_one).unwrap()
        );
        ...
        assert_eq!(
            ...
            triangulate::<Andrew>(base_sample_three).unwrap()
        );
        ...
        assert_eq!(
            ...
            triangulate::<Andrew>(base_sample_four).unwrap()
        );
    }
}
