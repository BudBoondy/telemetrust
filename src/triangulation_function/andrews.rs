...
impl TriangulationFunction for Andrew {
    fn triangulate(b: Vec<Bearing>) -> Result<Location, EqualAngleError> {
        ...
        let first = wi.iter().zip(si.iter()).map(|(w, s)| w * (s * s)).sum();
        let second = - wi.iter().zip(si.iter()).zip(ci.iter()).map(|((w, s), c)| w * s * c).sum::<f64>();
        let fourth = wi.iter().zip(ci.iter()).map(|(w, c )| w * c.powi(2)).sum::<f64>();
        let a =  Matrix::new(2, 2, vec![
            round_to_one(first), round_to_one(second),
            round_to_one(second), round_to_one(fourth)
        ]);
        let b_first: f64 = round_to_one(wi.iter().zip(si.iter()).zip(zi.iter()).map(|((w, s), z)| w * s * z).sum::<f64>());
        let b_second: f64 = - round_to_one(wi.iter().zip(ci.iter()).zip(zi.iter()).map(|((w, c), z)| w * c * z).sum::<f64>());
        ...
        while(x.abs()-x0.abs()).abs() > 0.0001 && (y.abs()-y0.abs()).abs() > 0.0001 && counter <= 999 {
            ...
            let mui = sub(&yi, y).iter().zip(sub(&xi, x).iter()).map(|(y, x)| y.atan2(*x)).collect::<Vec<f64>>();
            ...
            let kappa_inv = 2.0 * ( 1.0 - cw ) + (1.0 - cw).powi(2) * (0.48794 - 0.82905 * cw - 1.3915 * cw.powi(2)) / cw;
            let kappa = 1.0 / kappa_inv;
            let phi = subtract_vec(&theta, &mui);
            let t: Vec<f64> = (sub(&phi.iter()
                                        .map(|p| p.cos())
                                        .collect::<Vec<f64>>(), 1.0))
                                .iter()
            ...
            let psi_a: Vec<f64> = t.iter().map(|te| match te.abs() < 1.5 * std::f64::consts::PI{
            true => 1.5 * (te/1.5).sin(),
            false => 0.0
            }).collect();
            wi = div_vec(&psi_a, &t);
            ...
            let first = wi.iter().zip(si.iter().zip(s_star.iter())).map(|(w, (s, ss))| w * s * ss).sum();
            let second = -1_f64 * wi.iter().zip(si.iter().zip(c_star.iter())).map(|(w, (s, cs))| w * s * cs).sum::<f64>();
            let third = -1_f64 * wi.iter().zip(ci.iter().zip(s_star.iter())).map(|(w, (c, ss))| w * c * ss).sum::<f64>();
            let fourth =  wi.iter().zip(ci.iter().zip(c_star.iter())).map(|(w, (c, cs))| w * c * cs).sum();
            let q =  Matrix::new(2, 2, vec![
                first,  third,
                second, fourth
            ]);
...
mod tests {
    ...
    fn test_triangulation(){
        ...
        assert_eq!(Location{lat: -103.10058, lon: 33.77577}, triangulate::<Andrew>(base_sample_one).unwrap() );
        ...
        // fourth test
        assert_eq!(Location{lat: -99.87076, lon: 36.50256}, triangulate::<Andrew>(base_sample_three).unwrap());
        ...
        assert_eq!(Location{lat: -99.86714, lon: 36.50754}, triangulate::<Andrew>(base_sample_four).unwrap());
    }
}
