...
impl TriangulationFunction for Huber {
    ...
    fn triangulate(b: Vec<Bearing>) -> Result<Location, TriangulationError> {  
      ...
      let second = -1_f64 *  wi.iter().zip(si.iter().zip(ci.iter())).map(|(w, (s, c))| w * s * c).sum::<f64>();
      ...
      let b_first = round_to_one(wi.iter().zip(si.iter().zip(zi.iter())).map(|(w, (s, z))| w * s * z).sum());
      let b_second = round_to_one(- wi.iter().zip(ci.iter().zip(zi.iter())).map(|(w, (c, z))| w * c * z).sum::<f64>());
    ...
    let psih: Vec<f64> = t.iter().map(|te| te.signum() * match te.abs() < 1.5{
      true => te.abs(),
      false => 1.5
    }).collect();
    wi = div_vec(&psih, &t);
...
mod tests {
    ...
    fn test_triangulation(){
        ...
        assert_eq!(Location{lat: -103.10058, lon: 33.77577}, triangulate::<Huber>(base_sample_one).unwrap() );
        ...
        assert_eq!(Location{lat: -103.07850, lon: 33.79587}, triangulate::<Huber>(base_sample_two).unwrap());
        ...
        assert_eq!(Location{lat: -99.87076, lon: 36.50256}, triangulate::<Huber>(base_sample_three).unwrap());
        ...
        assert_eq!(Location{lat: -99.86714, lon: 36.50754}, triangulate::<Huber>(base_sample_four).unwrap());
    }
}
