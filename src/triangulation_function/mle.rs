...
impl TriangulationFunction for MLE {
    fn triangulate(b: Vec<Bearing>) -> Result<Location, TriangulationError> {
        ...
        let a = Matrix::new(2, 2, vec![first, second_first, second_first, third_second]);
        let b = Vector::new(vec![
            si.iter().zip(zi.iter()).map(|(s, z)| s * z).sum(),
            -1_f64 * ci.iter().zip(zi.iter()).map(|(c, z)| c * z).sum::<f64>(),
        ]);
            ...
            && counter <= 999
        {
            ...
            let fsecond: f64 = -1_f64
                * si.iter()
                    .zip(c_star.iter())
                    .map(|(i, j)| i * j)
                    .sum::<f64>();

            let sfirst: f64 = -1_f64
                * ci.iter()
                    .zip(s_star.iter())
                    .map(|(s, c)| s * c)
                    .sum::<f64>();
...
mod tests {
    ...
    fn test_triangulation() {
        ...
        assert_eq!(
            ...
            triangulate::<MLE>(base_sample_one).unwrap()
        );
        ...
        assert_eq!(
            ...
            triangulate::<MLE>(base_sample_two).unwrap()
        );
        ...
        assert_eq!(
            ...
            triangulate::<MLE>(base_sample_three).unwrap()
        );
        ...
        assert_eq!(
            ...
            triangulate::<MLE>(base_sample_four).unwrap()
        );
    }
}
